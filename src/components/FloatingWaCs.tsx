...
export default function FloatingWaCs() {
  ...
  const csNum = content.waConfig.csWa || content.appConfig.waCS || "6282144995255";
  const waCSText = encodeURIComponent(
    content.waConfig.waMessageTemplate ||
    "Halo CS MAXY, saya mau bertanya mengenai Masterclass AI-Driven Content Creation (8 Pertemuan Hybrid) yang mulai 4 Agustus 2026."
  );

  const waUrl = content.waConfig.customWaLink && content.waConfig.customWaLink.trim() !== ""
    ? content.waConfig.customWaLink
    : `https://wa.me/${csNum}?text=${waCSText}`;
